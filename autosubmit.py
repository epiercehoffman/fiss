...
def check_entity_status_for_workflow(namespace, workspace, workflow, entity_name, entity_type, require_success=False):
  workflow_root_entity_type = _fapi_get_method_config(NAMESPACE, WORKSPACE, NAMESPACE, workflow).json()['rootEntityType']
...
def ready_to_submit(batch, current, previous, entity_type):
...
def auto_submit(current, previous, interval, comment, output_log, retry=True,
                ...
                memory_retry_multiplier=None, call_cache=True):
  num_batches = len(batches)
  to_retry = []
  current_root_entity_type = _fapi_get_method_config(NAMESPACE, WORKSPACE, NAMESPACE, current).json()['rootEntityType']
  ...
  with open(output_log, 'a') as out:
    ...
    for i, batch in enumerate(batches):
      batch_status = ready_to_submit(batch, current, previous, submission_entity_type)
      if batch_status == READY:
        ...
        if dry_run:
          ...
          sub_response = fiss_api.create_submission(NAMESPACE, WORKSPACE, NAMESPACE, current,
